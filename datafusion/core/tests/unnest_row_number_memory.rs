...
fn create_test_data_with_size(num_rows: usize, list_size: usize) -> Result<RecordBatch> {
    ...
    for i in 0..metadata.len() {
        ...
        let values: Vec<Option<i32>> =
            (start..start + list_size as i32).map(Some).collect();
    ...
    let schema = Arc::new(Schema::new(vec![
        ...
        Field::new("values_a", DataType::new_list(DataType::Int32, true), false),
        Field::new("values_b", DataType::new_list(DataType::Int32, true), false),
    ]));
...
async fn test_row_number_has_output_ordering() -> Result<()> {
    ...
    if let Some(ordering) = output_ordering {
        println!(
            "Output ordering present with {} expressions",
            ordering.len()
        );
        assert_eq!(
            ordering.len(),
            1,
            "Row number should have one ordering column"
        );
    } else {
...
async fn test_large_dataset_memory_efficiency() -> Result<()> {
    ...
    println!(
        "Created test data: {} rows, ~1M elements total",
        batch.num_rows()
    );
